...
const sourceRoot = path.join(repoRoot, "medical-production-authoring");
...
function auditPlayerText(context, field, value, options = {}) {
  ...
  if (options.doctorSpeech) {
    const prefix = policy.playerFacingLanguage.instructionPrefixesNotAcceptedAsDoctorSpeech
...
const report = {
  ...
  manifestConsistency: {
    packageVersionMatches: manifest.packageVersion === policy.input.packageVersion,
...
writeJson("generated/P8_SOURCE_AUDIT.json", report);
