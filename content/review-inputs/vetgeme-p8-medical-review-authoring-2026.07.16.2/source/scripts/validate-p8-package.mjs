...
const display = JSON.parse(fs.readFileSync(path.join(packageRoot, 'generated/P8_DISPLAY_LANGUAGE_AUDIT_2026.07.16.40.json'), 'utf8'));
...
const exactChecks = {
  ...
  investigationCount: display.counts.investigations === 1864 && display.counts.nullResults === 0,
  sourceAuditClean: audit.status === 'reviewable' && audit.counts.issues === 0 && manifest.gates.sourceAudit === 'reviewable',
  displayLanguageClean: display.status === 'passed' && display.counts.issues === 0 && manifest.gates.displayLanguageAudit === 'passed',
  correctionComplete: manifest.gates.correctionComplete === true,
...
const report = {
  ...
  displayDefectsRemainOpen: display.counts.issues,
