...
const operationalRoot = process.env.VETGEME_OPERATIONAL_ROOT || path.join(repoRoot, "operational-production-authoring-2026.07.16.2");
const researchCatalog = readJson(path.join(operationalRoot, "generated/p3/research-catalog.json"));
const usagePolicy = readJson(path.join(operationalRoot, "generated/p3/investigation-usage-policy.json"));
const economyCatalog = readJson(path.join(operationalRoot, "generated/p6/economy-catalog.json"));
const exactMapPath = path.join(packageRoot, "source/p5-exact-capability-resource-map.json");
const lifecyclePath = path.join(packageRoot, "source/p5-resource-lifecycle.json");
const handoffPath = path.join(packageRoot, "source/p5-handoff-contract.json");
const exactMap = fs.existsSync(exactMapPath) ? readJson(exactMapPath) : null;
const lifecycle = fs.existsSync(lifecyclePath) ? readJson(lifecyclePath) : null;
const handoffContract = fs.existsSync(handoffPath) ? readJson(handoffPath) : null;
  ...
  .map((capability) => {
    ...
    return {
      ...
      activationRequirements: economy.assetPolicy.startsOwned
        ? ["training_complete", "maintenance_current"]
        : ["p6_asset_owned", "delivery_complete", "training_complete", "maintenance_current"],
      runtimeResourceTemplate: {
...
const researchTasks = researchCatalog.research.map((research) => {
  ...
  return {
    ...
    protocolCapabilityIds: requirements.protocolCapabilityIds,
    physicalResourceIds: requirements.physicalResourceIds,
    safeRouteId: research.activationGate?.fallbackRouteId || research.fallback || "safe_referral",
    runtimeTemplate: {
      priority: 220,
...
const operationalPolicies = {
  ...
  handoffRuntimeContract: handoffContract || policy.handoffRuntimeContract,
  ...
  lifecycleInvariants: lifecycle?.invariants || null
};

const capabilityOperationsForOutput = exactMap?.capabilities || capabilityOperations;
const lifecycleSeedById = new Map((lifecycle?.resourceLifecycleSeeds || []).map((item) => [item.resourceId, item]));
const resourcesForOutput = allResources.map((resource) => {
  const seed = lifecycleSeedById.get(resource.resourceId);
  if (!seed) return resource;
  return {
    ...resource,
    startsActive: ["owned_delivered_ready", "owned_delivered_trained_maintained"].includes(seed.state),
    initialLifecycleState: seed.state,
    initialEvidenceIds: seed.evidenceIds,
    activationRequirements: seed.activationRequires
  };
});
...
writeJson("generated/capability-operations-map.json", {
  ...
  status: policy.status,
  runtimeEligible: false,
  capabilities: capabilityOperationsForOutput,
  supplementalCapabilities: exactMap?.supplementalCapabilities || []
});
...
if (lifecycle) {
  writeJson("generated/resource-lifecycle-catalog.json", {
    schemaVersion: 1,
    catalogId: "vetgeme-p5-resource-lifecycle-runtime-contract",
    catalogVersion: policy.catalogVersion,
    status: policy.status,
    runtimeEligible: false,
    commands: lifecycle.commands,
    roomAssets: lifecycle.roomAssets,
    startingInventory: lifecycle.startingInventory,
    startingEquipmentEvidence: lifecycle.startingEquipmentEvidence,
    resourceLifecycleSeeds: lifecycle.resourceLifecycleSeeds,
...
const manifest = {
  ...
  counts: {
    ...
    capabilitiesMapped: capabilityOperationsForOutput.length,
    supplementalCapabilitiesMapped: exactMap?.supplementalCapabilities?.length || 0,
  ...
  files: [
    ...
    "scripts/author-v2-lifecycle-contracts.mjs",
