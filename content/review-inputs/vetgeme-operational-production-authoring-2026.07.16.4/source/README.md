# VetGEME operational production authoring 2026.07.16.4

Полный авторский correction-пакет для P3, P4, P6 и P7. Он заменяет
operational `.3` как будущий author source, но **не изменяет и не уничтожает**
`.2` и `.3`: предыдущие версии остаются immutable evidence независимых
P1-аудитов.
...
- P3: устранён drift всех 361 `medicalResultAuthority`: generated projection
  теперь побайтно сохраняет author source
  `medical_family.presentation.investigations[].result_only`, а не сокращает
  его до другого namespace.
...
Ожидаемый результат: 64 checks, 10 000 seeded campaigns, не менее 295 000
