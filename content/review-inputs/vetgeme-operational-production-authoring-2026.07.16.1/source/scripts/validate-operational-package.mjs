...
const medicalManifest = readJson(path.join(repoRoot, "medical-production-authoring/MANIFEST.json"));
...
const medicalFamilies = medicalManifest.families.map((entry) => readJson(path.join(repoRoot, "medical-production-authoring", entry.path)));
...
const report = {
  ...
  limitations: [
    ...
    "P5 runtime resource IDs require the programmer's exact catalog adapter; no IDs were invented here.",
