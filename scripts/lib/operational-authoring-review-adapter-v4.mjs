...
export const OPERATIONAL_AUTHORING_REVIEW_ADAPTER_V4_VERSION =
  "vetgeme-operational-review-adapter@2026.07.16.4";
export const OPERATIONAL_AUTHORING_REVIEW_ADAPTER_V4_STATE_VERSION = 1;

const P7_RECORD_COUNT = 93;
const P3_MEDICAL_RESULT_AUTHORITY = "medical_family.presentation.investigations[].result_only";
const P3_MEDICAL_RESULT_AUTHORITY_DRIFT_BLOCKER =
  "p3_medical_result_authority_projection_drift";
const SHA256_PATTERN = /^[a-f0-9]{64}$/u;

function fail(message) {
  throw new Error(`Operational authoring v4 review adapter rejected: ${message}`);
}
...
function sameCanonicalJson(left, right) {
  return canonicalOperationalV4Json(left) === canonicalOperationalV4Json(right);
}

export function canonicalOperationalV4Json(value) {
  ...
  if (Array.isArray(value)) {
    return `[${value.map((item) => canonicalOperationalV4Json(item)).join(",")}]`;
  }
  checkObject(value, "canonical JSON value");
  const keys = Object.keys(value).sort();
  return `{${keys.map((key) => `${JSON.stringify(key)}:${canonicalOperationalV4Json(value[key])}`).join(",")}}`;
}

export function sha256CanonicalOperationalV4(value) {
  return createHash("sha256").update(canonicalOperationalV4Json(value), "utf8").digest("hex");
}
...
function buildP3Context(input) {
  ...
  check(investigationUsagePolicy.catalogVersion === "2026.07.16.4", "P3 usage policy version mismatch");
  ...
  check(p3Policy.policyVersion === "2026.07.16.4", "P3 policy version mismatch");
  ...
  check(researchCatalog.catalogVersion === "2026.07.16.4", "P3 research catalog version mismatch");
  ...
  check(providerCatalog.catalogVersion === "2026.07.16.4", "P3 provider catalog version mismatch");
  ...
  const medicalResultAuthorityProjectionDrift = [];
  for (const [researchId, research] of researchById) {
    const explicitResearch = explicitResearchById.get(researchId);
    const reasons = [];
    if (explicitResearch.medicalResultAuthority !== P3_MEDICAL_RESULT_AUTHORITY) {
      reasons.push("explicit authority is missing or is not the exact medical-family namespace");
    }
    if (research.medicalResultAuthority !== P3_MEDICAL_RESULT_AUTHORITY) {
      reasons.push("generated authority is missing or is not the exact medical-family namespace");
    }
    if (research.medicalResultAuthority !== explicitResearch.medicalResultAuthority) {
      reasons.push("generated authority differs from its explicit authority source");
    }
    if (research.operationalPolicyMayGenerateResult !== false) {
      reasons.push("operational result generation is not explicitly forbidden");
    }
    if (reasons.length > 0) {
      medicalResultAuthorityProjectionDrift.push({ researchId, reasons });
    }
  }
  check(medicalResultAuthorityProjectionDrift.length === 0,
    `${P3_MEDICAL_RESULT_AUTHORITY_DRIFT_BLOCKER}: ${medicalResultAuthorityProjectionDrift.length} ` +
    "P3 research record(s) violate the exact medical-result authority contract");
  ...
  return {
    ...
    audit: deepFreeze({
      ...
      medicalResultAuthority: P3_MEDICAL_RESULT_AUTHORITY,
      medicalResultAuthorityProjectionValidated: true,
      medicalResultAuthorityProjectionDrift: medicalResultAuthorityProjectionDrift.length,
...
export function validateOperationalP3AdapterBundleV4(input) {
...
export function validateOperationalP4AdapterBundleV4(input) {
...
function buildP7Context(input) {
  ...
  for (const section of digestContract.resolverSections) {
    for (const payload of checkArray(evidenceResolver[section], `P7 resolver section ${section}`)) {
      ...
      records.push({ section, recordId, contentSha256: sha256CanonicalOperationalV4(payload), payload });
    }
  }
  ...
  const expectedResolverEnvelope = {
    ...
    records,
    resolverDigest: sha256CanonicalOperationalV4({ adapterVersion: digestContract.adapterVersion, records }),
  };
  ...
  for (const [section, [kind, idField, envelopeType]] of Object.entries(sourceEnvelopeSections)) {
    ...
    envelopes.forEach((envelope, index) => {
      ...
      check(envelope.catalogRef?.contentSha256 === sha256CanonicalOperationalV4(envelope.payload),
  ...
  const activationDigest = sha256CanonicalOperationalV4(expectedActivationInput);
  ...
  const mutationDigest = sha256CanonicalOperationalV4(mutationProbe);
...
export function validateOperationalP7DigestBundleV4(input) {
...
export function validateOperationalP5ReservationGateV4(input) {
...
function emptyState() {
  return {
    schemaVersion: OPERATIONAL_AUTHORING_REVIEW_ADAPTER_V4_STATE_VERSION,
    adapterVersion: OPERATIONAL_AUTHORING_REVIEW_ADAPTER_V4_VERSION,
    reviewOnly: true,
    orders: [],
    commands: [],
  };
...
function normalizeAndValidateState(p3, value) {
  checkExactKeys(value, ["schemaVersion", "adapterVersion", "reviewOnly", "orders", "commands"],
    "adapter state");
  check(value.schemaVersion === OPERATIONAL_AUTHORING_REVIEW_ADAPTER_V4_STATE_VERSION,
    "adapter state schemaVersion mismatch");
  check(value.adapterVersion === OPERATIONAL_AUTHORING_REVIEW_ADAPTER_V4_VERSION,
  ...
  for (const [index, order] of orders.entries()) {
    ...
    const contractDigest = sha256CanonicalOperationalV4({
      usageId: usage.usageId,
      sourceUrgency: usage.sourceUrgency,
  ...
  for (const [index, record] of commands.entries()) {
    ...
    check(record.fingerprint === sha256CanonicalOperationalV4(command),
...
export function createOperationalAuthoringReviewAdapterV4(input) {
  checkObject(input, "operational v4 adapter input");
  ...
  function placeInvestigationOrder(state, rawCommand) {
    ...
    const fingerprint = sha256CanonicalOperationalV4(command);
    ...
    const order = {
      ...
      contractDigest: sha256CanonicalOperationalV4({
        usageId: usage.usageId,
        sourceUrgency: usage.sourceUrgency,
        sourceClassification: usage.sourceClassification,
        urgencyResolution: usage.urgencyResolution,
...
export function createOperationalAuthoringReviewAdapterV4FromReviewInputs(
  ...
  return createOperationalAuthoringReviewAdapterV4({
