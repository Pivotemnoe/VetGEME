...
export const OPERATIONAL_REVIEW_INPUT_V3_ID = "vetgeme-operational-production-authoring";
export const OPERATIONAL_REVIEW_INPUT_V3_VERSION = "2026.07.16.3";
export const OPERATIONAL_REVIEW_INPUT_V3_ROOT =
  "content/review-inputs/vetgeme-operational-production-authoring-2026.07.16.3";
export const OPERATIONAL_REVIEW_INPUT_V3_CONTEXT = "review";
export const OPERATIONAL_REVIEW_INPUT_V3_STATUS = "author_complete_validation_passed";
export const BASELINE_MANIFEST_PATH = "content/packs/tier-01-v2/clinical/tier-01/manifest.json";

const OPERATIONAL_PROVENANCE_SHA256 = "105b5c037652eac183a29af7e4afdb6f044ebce6811e69a48199b2e7a37eb986";
const OPERATIONAL_AGGREGATE_SHA256 = "20e2b8768b1be7cdacefb11fdb1b2dc1139d0f007097c99188e0706bab685284";
const OPERATIONAL_ARCHIVE_SHA256 = "5abee5242467e703561e0de2eefbb5050345448c90b21b97761d7ee60540a1df";
...
function fail(message) {
  throw new Error(`Operational authoring v3 review input validation failed: ${message}`);
}
...
function validateCatalogHeader(
  ...
  label,
  expectedStatus,
  expectedVersion = OPERATIONAL_REVIEW_INPUT_V3_VERSION,
) {
...
export function findForbiddenOperationalV3AuthorityKeys(value, location = "document", found = []) {
  if (Array.isArray(value)) {
    value.forEach((item, index) => findForbiddenOperationalV3AuthorityKeys(item, `${location}[${index}]`, found));
    return found;
  }
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    findForbiddenOperationalV3AuthorityKeys(child, `${location}.${key}`, found);
  }
  return found;
}

export function validateOperationalV3SafeRouteProviderTarget(payload, label = "safe route") {
...
export function validateOperationalReviewInputV3Registration(registration) {
  check(isObject(registration), "review input registration must be an object");
  check(registration.reviewInputId === OPERATIONAL_REVIEW_INPUT_V3_ID, "unexpected reviewInputId");
  check(registration.reviewInputVersion === OPERATIONAL_REVIEW_INPUT_V3_VERSION, "unexpected reviewInputVersion");
  check(registration.kind === "operational_authoring", "kind must be operational_authoring");
  check(registration.packageId === OPERATIONAL_REVIEW_INPUT_V3_ID, "packageId mismatch");
  check(registration.packageVersion === OPERATIONAL_REVIEW_INPUT_V3_VERSION, "packageVersion mismatch");
  check(registration.root === OPERATIONAL_REVIEW_INPUT_V3_ROOT, "versioned operational root mismatch");
  ...
  check(registration.status === OPERATIONAL_REVIEW_INPUT_V3_STATUS, "status mismatch");
  ...
  check(
    registration.authorSourceSupersession.supersedesReviewInputVersion === "2026.07.16.2",
    "operational .3 must supersede .2 only as an author source",
  );
  check(
    sameJson(registration.authorSourceSupersession.scopes, ["p3", "p4", "p7"]),
    "operational .3 author-source supersession scope mismatch",
  );
  check(registration.authorSourceSupersession.priorVersionPreserved === true, "operational .2 must remain preserved");
  check(
    registration.authorSourceSupersession.p5P6ReservationAuthorityJoined === false,
    "operational .3 must not claim P5/P6 reservation authority",
  );
...
export function resolveOperationalAuthoringReviewInputV3(registry, options = {}) {
  ...
  check(
    options.context === OPERATIONAL_REVIEW_INPUT_V3_CONTEXT,
    `context ${String(options.context)} is forbidden; review is the only allowed context`,
  );
  const requestedId = options.reviewInputId || OPERATIONAL_REVIEW_INPUT_V3_ID;
  const requestedVersion = options.reviewInputVersion || OPERATIONAL_REVIEW_INPUT_V3_VERSION;
  check(requestedVersion === OPERATIONAL_REVIEW_INPUT_V3_VERSION, "v3 loader refuses non-.3 operational input");
  ...
  return validateOperationalReviewInputV3Registration(registration);
}

export function validateOperationalV3SourceProvenance(
  registration,
  provenance,
  sourceFiles,
  sourceBytesByPath,
  ...
  check(
    provenance.sourceArchive === "operational-production-authoring-2026.07.16.3.zip",
    `${identity}: source archive mismatch`,
  );
  check(
    provenance.sourceDirectory === "operational-production-authoring-2026.07.16.3",
  ...
  check(
    provenance.archive.checksumPath === "operational-production-authoring-2026.07.16.3.zip.sha256",
...
function validateP3(documents, capabilityById) {
  ...
  validateCatalogHeader(exact, "vetgeme-p3-exact-source-crosswalk", "P3 exact source crosswalk", "author_complete_programmer_adapter_required");
  ...
  for (const record of research.research) {
    ...
    check(record.operationalPolicyMayGenerateResult === false, `${record.researchId}: operational result generation must remain forbidden`);
  ...
  return {
    ...
    blockers: [
      makeBlocker(
        ...
        "All generated P3 research records shorten the explicit medical-result authority path; runtime result ownership remains blocked until a new author package corrects the projection.",
        {
...
function validateP7(documents) {
  ...
  check(digestContract.contractVersion === OPERATIONAL_REVIEW_INPUT_V3_VERSION, "P7 activation digest contract version mismatch");
  ...
  const buildProjection = (resolverSource) => {
    ...
    const resolverEnvelope = {
      ...
      catalogVersion: OPERATIONAL_REVIEW_INPUT_V3_VERSION,
...
export function validateOperationalAuthoringPackageV3({
  registration,
  manifest,
  sourceFiles,
  sourceBytesByPath,
  ...
  validateOperationalReviewInputV3Registration(registration);
  const capabilityById = validateCapabilityRegistry(registration, capabilityRegistry, capabilityBytes);
  check(isObject(manifest), "operational .3 manifest is missing");
  check(manifest.schemaVersion === 1, "operational .3 manifest schemaVersion must be 1");
  check(manifest.packageId === registration.packageId, "operational .3 manifest packageId mismatch");
  check(manifest.packageVersion === registration.packageVersion, "operational .3 manifest packageVersion mismatch");
  check(manifest.status === registration.status, "operational .3 manifest status mismatch");
  check(manifest.runtimeEligible === false, "operational .3 manifest runtimeEligible must remain false");
  check(sameStrings(manifest.activationRequires, EXPECTED_ACTIVATION_REQUIREMENTS), "operational .3 activation requirements changed");
  check(manifest.boundaries?.runtimeChanged === false, "operational .3 must not claim runtime changes");
  check(manifest.boundaries?.designChanged === false, "operational .3 must not claim design changes");
  check(manifest.boundaries?.saveSchemaChanged === false, "operational .3 must not claim save-schema changes");
  check(manifest.boundaries?.medicalTruthAuthoredHere === false, "operational .3 must not author medical truth");
  check(manifest.boundaries?.existingThirtyCardPoolChanged === false, "operational .3 must preserve the current 30-card pool");
  check(manifest.boundaries?.p5CatalogAuthoredHere === false, "operational .3 must not claim P5 catalog authority");
  check(manifest.sources?.medicalPackageId === "vetgeme-medical-production-authoring", "operational .3 medical source ID mismatch");
  check(manifest.sources?.medicalPackageVersion === MEDICAL_REVIEW_INPUT_V40_VERSION, "operational .3 medical source version mismatch");
  ...
  check(isObject(manifest.counts), "operational .3 manifest counts are missing");
  ...
  check(sameStrings(Object.keys(manifest.counts), Object.keys(manifestExpectedCounts)), "operational .3 manifest count fields mismatch");
  for (const [field, expected] of Object.entries(manifestExpectedCounts)) {
    check(manifest.counts[field] === expected, `operational .3 manifest count ${field} mismatch`);
  }
  checkArray(manifest.files, "operational .3 manifest files");
  check(manifest.files.length === EXPECTED_COUNTS.sourceFiles, "operational .3 manifest must list 40 files");
  checkUnique(manifest.files, "operational .3 manifest files");
  manifest.files.forEach((file) => checkSafePath(file, `manifest file ${file}`));
  check(sameStrings(manifest.files, sourceFiles), "operational .3 manifest and provenance file sets differ");
  check(sameStrings(REQUIRED_JSON_PATHS, Object.keys(documents)), "operational .3 parsed JSON file set mismatch");
  for (const path of AUTHORITATIVE_EXACT_PATHS) {
    const forbidden = findForbiddenOperationalV3AuthorityKeys(documents[path], path);
    check(
      forbidden.length === 0,
  ...
  check(validationReport.checks.length === 62, "bundled author validation check count mismatch");
  ...
  check(p1CorrectionMatrix.findings.length === 5, "P1 correction matrix finding count mismatch");
  check(
    sameStrings(
  ...
  check(baselineCrosswalkMatches.length === 0, "operational .3 contains an unapproved current 30-card crosswalk");
  check(!sourceFiles.some((path) => /activation.?manifest/iu.test(path)), "operational .3 must not contain an activation manifest");
...
export async function loadOperationalAuthoringReviewInputV3FromReader(reader, registry, options = {}) {
  ...
  const registration = resolveOperationalAuthoringReviewInputV3(registry, options);
  ...
  const sourceIntegrity = validateOperationalV3SourceProvenance(
    registration,
    provenance,
    sourceFiles,
    sourceBytesByPath,
  ...
  const audit = validateOperationalAuthoringPackageV3({
    registration,
    manifest,
    sourceFiles,
    sourceBytesByPath,
  ...
  return deepFreeze({
    loadContext: OPERATIONAL_REVIEW_INPUT_V3_CONTEXT,
    reviewOnly: true,
...
export async function loadOperationalAuthoringReviewInputV3(projectRoot, options = {}) {
  ...
  return loadOperationalAuthoringReviewInputV3FromReader(reader, registry, options);
}
