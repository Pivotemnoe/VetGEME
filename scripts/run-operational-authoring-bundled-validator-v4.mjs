...
export async function runOperationalAuthoringBundledValidatorV4({ rebuild = false } = {}) {
  const before = await loadOperationalAuthoringReviewInputV4(projectRoot, { context: "review" });
  assert.deepEqual(
    before.blockers.map((blocker) => blocker.id),
    EXPECTED_BLOCKERS,
    "operational .4 must expose only the activation and exact P5 join blockers",
  );
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vetgeme-operational-authoring-v4-validator-"));
  const operationalRoot = path.join(temporaryRoot, "operational-production-authoring-2026.07.16.4");
  ...
  try {
    ...
    assert.equal(validationReport.checks.length, 64);
    ...
    assert.equal(
      correctionMatrix.findings.find((finding) => (
        finding.id === "p3_medical_result_authority_projection_drift"
      ))?.evidence?.explicitGeneratedMismatches,
      0,
      "operational .4 must close the 361-record medical-result authority projection drift",
    );
    ...
    const after = await loadOperationalAuthoringReviewInputV4(projectRoot, { context: "review" });
    assert.equal(
      before.sourceIntegrity.aggregateSha256,
      after.sourceIntegrity.aggregateSha256,
      "tracked operational .4 source changed while running the bundled validator",
    );
    assert.deepEqual(
      after.blockers.map((blocker) => blocker.id),
      EXPECTED_BLOCKERS,
      "operational .4 blocker set changed while running the bundled validator",
    );
    return Object.freeze({
      ...
      medicalResultAuthorityProjectionMismatches: 0,
...
if (process.argv[1] && path.resolve(process.argv[1]) === scriptPath) {
  ...
  console.log(JSON.stringify(await runOperationalAuthoringBundledValidatorV4({
