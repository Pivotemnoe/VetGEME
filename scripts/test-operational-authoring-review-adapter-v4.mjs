...
import {
  OPERATIONAL_AUTHORING_REVIEW_ADAPTER_V4_STATE_VERSION,
  OPERATIONAL_AUTHORING_REVIEW_ADAPTER_V4_VERSION,
  createOperationalAuthoringReviewAdapterV4,
  createOperationalAuthoringReviewAdapterV4FromReviewInputs,
  sha256CanonicalOperationalV4,
  validateOperationalP3AdapterBundleV4,
  validateOperationalP4AdapterBundleV4,
  validateOperationalP5ReservationGateV4,
  validateOperationalP7DigestBundleV4,
} from "./lib/operational-authoring-review-adapter-v4.mjs";
...
const p3Audit = validateOperationalP3AdapterBundleV4(p3Bundle(input));
assert.deepEqual(p3Audit, {
  ...
  medicalResultAuthorityProjectionValidated: true,
  medicalResultAuthorityProjectionDrift: 0,
  operationalPolicyMayGenerateResult: false,
  blockers: [],
});

const p4Audit = validateOperationalP4AdapterBundleV4(p4Bundle(input));
...
const p7Audit = validateOperationalP7DigestBundleV4(p7Bundle(input));
...
assert.deepEqual(validateOperationalP5ReservationGateV4({
...
const adapter = createOperationalAuthoringReviewAdapterV4(input);
assert.equal(adapter.adapterVersion, OPERATIONAL_AUTHORING_REVIEW_ADAPTER_V4_VERSION);
...
assert.equal(adapter.audit.p3.medicalResultAuthority,
  "medical_family.presentation.investigations[].result_only");
assert.equal(adapter.audit.p3.medicalResultAuthorityProjectionDrift, 0);
assert.equal(adapter.audit.p3.operationalPolicyMayGenerateResult, false);
assert.deepEqual(adapter.audit.p3.blockers, []);
...
assert.equal(initial.schemaVersion, OPERATIONAL_AUTHORING_REVIEW_ADAPTER_V4_STATE_VERSION);
...
const noDynamicResolverAdapter = createOperationalAuthoringReviewAdapterV4({
  ...input,
...
const invalidDynamicResolverAdapter = createOperationalAuthoringReviewAdapterV4({
  ...input,
  medicalUrgencyResolvers: {
    ...input.medicalUrgencyResolvers,
...
const noProviderCalendarAdapter = createOperationalAuthoringReviewAdapterV4({
  ...input,
...
assert.throws(() => validateOperationalP3AdapterBundleV4(p3Bundle(mutatedP3)),
...
assert.throws(() => validateOperationalP3AdapterBundleV4(p3Bundle(incompleteP3)),
...
assert.throws(() => validateOperationalP3AdapterBundleV4(p3Bundle(approximateP3)),
  /forbidden approximate\/default runtime authority/u);

const missingP3MedicalAuthority = clone(input);
delete missingP3MedicalAuthority.explicitResearchRoutes.researchRoutes[0].medicalResultAuthority;
assert.throws(() => validateOperationalP3AdapterBundleV4(p3Bundle(missingP3MedicalAuthority)),
  /p3_medical_result_authority_projection_drift: 1 P3 research record/u,
  "missing explicit medical-result authority must fail closed");

const shortenedP3MedicalAuthority = clone(input);
shortenedP3MedicalAuthority.researchCatalog.research[0].medicalResultAuthority =
  "family.presentation.investigations[].result_only";
assert.throws(() => validateOperationalP3AdapterBundleV4(p3Bundle(shortenedP3MedicalAuthority)),
  /p3_medical_result_authority_projection_drift: 1 P3 research record/u,
  "a shortened medical-result authority namespace must fail closed");

const unknownP3MedicalAuthority = clone(input);
unknownP3MedicalAuthority.researchCatalog.research[0].medicalResultAuthority =
  "unknown.presentation.investigations[].result_only";
assert.throws(() => validateOperationalP3AdapterBundleV4(p3Bundle(unknownP3MedicalAuthority)),
  /p3_medical_result_authority_projection_drift: 1 P3 research record/u,
  "an unknown medical-result authority namespace must fail closed");

const openedP3ResultGeneration = clone(input);
openedP3ResultGeneration.researchCatalog.research[0].operationalPolicyMayGenerateResult = true;
assert.throws(() => validateOperationalP3AdapterBundleV4(p3Bundle(openedP3ResultGeneration)),
  /p3_medical_result_authority_projection_drift: 1 P3 research record/u,
  "operational result generation must remain forbidden");
...
assert.throws(() => validateOperationalP4AdapterBundleV4(p4Bundle(mutatedP4)),
...
assert.throws(() => validateOperationalP4AdapterBundleV4(p4Bundle(crossPresentationFactSwap)),
...
assert.throws(() => validateOperationalP4AdapterBundleV4(p4Bundle(collapsedP4)),
...
assert.throws(() => validateOperationalP4AdapterBundleV4(p4Bundle(missingSafeRouteProvider)),
...
assert.throws(() => validateOperationalP4AdapterBundleV4(p4Bundle(ambiguousSafeRouteProvider)),
  /exactly one provider resolver or provider ID/u);

const openedP7Activation = clone(input);
openedP7Activation.evidenceResolver.activation.programmerAdapterValidated = true;
assert.throws(() => validateOperationalP7DigestBundleV4(p7Bundle(openedP7Activation)),
...
assert.throws(() => validateOperationalP7DigestBundleV4(p7Bundle(mutatedOuterCampaign)),
...
assert.throws(() => validateOperationalP7DigestBundleV4(p7Bundle(mutatedOuterAuthority)),
...
assert.throws(() => validateOperationalP7DigestBundleV4(p7Bundle(mutatedEmbeddedDigestContract)),
...
assert.throws(() => validateOperationalP7DigestBundleV4(p7Bundle(mutatedResolver)),
...
changedResolverRecord.contentSha256 = sha256CanonicalOperationalV4(changedResolverRecord.payload);
const rebuiltResolverDigest = sha256CanonicalOperationalV4({
...
assert.notEqual(
  sha256CanonicalOperationalV4(rebuiltResolverWithStaleActivation.directorCatalog.activationDigestInput),
...
assert.throws(() => validateOperationalP7DigestBundleV4(p7Bundle(rebuiltResolverWithStaleActivation)),
  /activation digest mismatch/u);

const mutatedDay = clone(input);
mutatedDay.dayCatalog.days[0].title += " mutated";
assert.throws(() => validateOperationalP7DigestBundleV4(p7Bundle(mutatedDay)),
...
assert.throws(() => validateOperationalP7DigestBundleV4(p7Bundle(mutatedEnvelope)),
...
assert.throws(() => validateOperationalP7DigestBundleV4(p7Bundle(mutatedActivationDigest)),
  /activation digest mismatch/u);

const openedP5Gate = clone(input.resourceCrosswalk);
openedP5Gate.reservationAuthority = true;
assert.throws(() => validateOperationalP5ReservationGateV4({ resourceCrosswalk: openedP5Gate }),
...
const immutableAdapter = createOperationalAuthoringReviewAdapterV4(inputMutationProbe);
...
const medical = loadMedicalFamilies();
const fromReviewInputs = createOperationalAuthoringReviewAdapterV4FromReviewInputs(
...
assert.equal(
  sha256CanonicalOperationalV4(input.directorCatalog.activationDigestInput),
...
console.log(JSON.stringify({
  ...
  medicalResultAuthorityProjectionVerified: true,
}, null, 2));
